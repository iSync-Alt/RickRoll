...
client.on("ready", () => {
  ...
  client.user.setPresence({ activity:{ name: `"rhelp"`, type: "WATCHING"}, status: "dnd" })
...
if(message.content.toLowerCase() === `${prefix}beg`) {
	let balance = await db.get(`wallet_${message.author.id}`)
	let bank = await db.get(`bank_${message.author.id}`)
	...
	let currency = "💸"
	if(balance > 0) balance = (balance + [Math.floor(Math.random() * 5)])
	let MoneyEmbed = new Discord.MessageEmbed()
.setTitle(`${message.author.username}`)
.setDescription(`***Wallet: ${balance} ${currency}\nBank: ${bank} ${currency}***`)
.setColor("BLUE")
...
if(message.content.toLowerCase() === prefix + "help mod") {
	...
	if(message.content.toLowerCase() === prefix + "server") {
		...
		.setTimestamp()
		message.channel.scnd(serverinfoembed)
		.then((sentMessage) => sentMessage.react("✔"));
	}
